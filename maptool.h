...
private:
	...
	vector<tagIsometricTile*> _vIsoTiles;			// iso타일 담아 둘 벡터
	...
	vector<tagSampleTile*> _vSampleObject2;			// 표시되는 오브젝트 담아둘 벡터2
	...
	tagTabButton _terrainBtn, _objectBtn[2];		// Terrain Object 선택 버튼
	tagButton _returnBtn;							// 메인으로 돌아가는 버튼
	
	// RECT _focus;									// 전역으로 빼서 주석처리함 포커스 ( 클리핑 검출용 카메라 렉트 )
