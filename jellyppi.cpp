...
HRESULT jellyppi::init()
{
	
	_image = IMAGEMANAGER->addPngFrameImage("젤리삐_MOVE", "image\\enemy\\젤리삐_이동.png", 180, 176, 5, 4, false, false);
	IMAGEMANAGER->addPngFrameImage("젤리삐_ATTACK", "image\\enemy\\젤리삐_공격.png", 648, 176, 18, 4, false, false);
	IMAGEMANAGER->addPngFrameImage("젤리삐_COLLISION", "image\\enemy\\젤리삐_충돌.png", 108, 176, 3, 4, false, false);
...
void jellyppi::release()
{

}

void jellyppi::update()
{
...
void jellyppi::render()
{
	...
	char str[128];
	sprintf_s(str, " %d / %d", _target.x, _target.y);
	TextOut(getMemDC(), _rc.left - _focus.left, _rc.top - _focus.top, str, strlen(str));
}

void jellyppi::attack()
{
	if (_isAttack)
	{
		_state = E_ATTACK;
...
void jellyppi::checkDirection(float angle)
{
...
void jellyppi::setMinion(int cx, int by)
{

	strcpy_s(_enemyName, "젤리삐");
...
void jellyppi::setImage()
{
	switch (_state)
	{
	case E_MOVE:
		_image = IMAGEMANAGER->findImage("젤리삐_MOVE");
		break;
	case E_ATTACK:	
		_image = IMAGEMANAGER->findImage("젤리삐_ATTACK");
		break;
	case E_COLLISION:		
		_image = IMAGEMANAGER->findImage("젤리삐_COLLISION");
		break;
	default:
		break;
	}
...
void jellyppi::addDamage(int damage)
{
